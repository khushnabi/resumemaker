<template>
    <div>
                ...
                <Modal v-model="isDeletedModel" width="360">
                    ...
                    <div slot="footer">
                        <Button
                            ...
                            size="large"
                            long
                            @click="deleteExp(expDeleteData.id)"
                            :loading="expDeleting"
        ...
        <div v-if="experiance.job_title" class="show-data">
            <p @click="isAddingExp = !isAddingExp" class="link-para">
                {{ experiance.job_title }}
            </p>
        </div>
        ...
        <div v-if="isAddingExp">
            ...
            <div>
                ...
                >
                    ...
                    <FormItem label="Desc">
                        <Input
                            v-model="experiance.description"
                            @input="inputEvent()"
                            type="textarea"
                    ...
                    <FormItem>
                        <div class="button-wrap">
                            <Button
                                ...
                                type="primary"
                                :loading="expsending"
                                @click="handleSubmit('experiance')"
                                >edit experiances</Button
                            >
        ...
        <div>
            <div class="open-form" @click="addExperiance">
                <div class="add">
                    <Icon color="#037bf8" size="16" type="md-add" />
                </div>
                Add Employment
            </div>
        </div>
        ...
        <div>
            <p class="small-para">
                In this section, list related employment experience in your last
                10 years along with the dates. Mention the most recent
                employment first.
            </p>
        </div>
    </div>
        ...
        return {
            ...
            ruleValidate: {
                ...
                start_at: [
                    {
                        required: false,
                        message: "The start cannot be empty",
                        trigger: "blur"
                    }
                ],
                ...
                end_at: [
                    {
                        required: false,
                        message: "Please select the end at",
                        trigger: "blur"
                    }
                ]
            }
    ...
    methods: {
        inputEvent() {
            if (this.experiance.start_at) {
                this.experiance.start_at = moment(
                    String(this.experiance.start_at)
                ).format("YYYY-MM-DD");
                console.log(this.experiance.end_at);
            }

            if (this.experiance.end_at) {
                this.experiance.end_at = moment(
                    String(this.experiance.end_at)
                ).format("YYYY-MM-DD");
            } else {
                    ...
                    if (this.expEdit) {
                        try {
                            ...
                            const { res } = await Axios.put(
                                `/api/resumes/${this.resumeId}/experiences/${this.editId}`,
                                this.experiance
                            );
                        ...
                        try {
                            ...
                            const res = await Axios.post(
                                `/api/resumes/${this.resumeId}/experiences`,
                                this.experiance
                            );
                            await this.getResumeData();
                            this.$Message.success("Success!");
        ...
        async deleteExp(id) {
            ...
            const { res } = await Axios.delete(
                `/api/resumes/${this.resumeId}/experiences/${id}`
            );
            this.removeData();
            await this.getResumeData();
            this.expDeleteData = {};
        ...
        async editExp(id) {
            ...
            this.resume.experiences = this.resume.experiences.filter(
                arr => arr.id !== id
            );
        }
    }
