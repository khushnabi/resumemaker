<template>
  <div>
        ...
        <Modal v-model="isDeletedModel" width="360">
          ...
          <div slot="footer">
            <Button
              ...
              size="large"
              long
              @click="deleteCustom(deleteCustomData.id)"
              :loading="customDeleting"
    ...
    <div>
      <div class="open-form" @click="addCustom">
  ...
  methods: {
    inputEvent() {
      ...
      if (this.custom.start_at) {
        this.custom.start_at = moment(String(this.custom.start_at)).format(
          "YYYY-MM-DD"
        );
      }
    },
    ...
    async editCustom(id) {
      ...
      const { data } = await Axios.get(
        `/api/resumes/${this.resumeId}/customs/${id}`
      );
      this.addingcustom = true;
