  ...
  <div>
    <div v-if="educationData.length > 0">
      <div v-for="(eduData, i) in educationData" :key="i">
        <div class="show-data">
          <p class="link-para">
            <span @click="editEdu(eduData.id)"> {{ eduData.school }}</span>
            <span
              ><Icon
                class="delete"
                size="20"
        ...
        <Modal v-model="isDeletedModel" width="360">
          ...
          <div slot="footer">
            <Button
              ...
              size="large"
              long
              @click="deleteEdu(eduDeleteData.id)"
              :loading="eduDeleting"
              :disabled="eduDeleting"
              >Delete</Button
            >
          </div>
        </Modal>
    ...
    <div v-if="education.school" class="show-data">
      <p @click="isAddingEdu = !isAddingEdu" class="link-para">
        {{ education.school }}
      </p>
    </div>
    ...
    <div></div>
    <div>
      <div class="open-form" @click="addEducation">
        <div class="add"><Icon color="#037bf8" size="16" type="md-add" /></div>
        Add Education
      </div>
      <div>
        <p class="small-para">
          In this section, list your level education; include any degrees and
          educational achievements, if relevant. Include the dates of your
          achievements.
        </p>
      </div>
    </div>
  ...
  methods: {
    inputEvent() {
      ...
      if (this.education.graduated_at) {
        this.education.graduated_at = moment(
          String(this.education.graduated_at)
        ).format("YYYY-MM-DD");
      }
    },
    getData() {
          ...
          if (this.editId) {
            ...
            try {
              ...
              const res = await Axios.put(
                `/api/resumes/${this.resumeId}/educations/${this.editId}`,
                this.education
              );
              await this.getResumeData();
            ...
            try {
              ...
              const res = await Axios.post(
                `/api/resumes/${this.resumeId}/educations`,
                this.education
              );
              await this.getResumeData();
    ...
    async editEdu(id) {
      ...
      const { data } = await Axios.get(
        `/api/resumes/${this.resumeId}/educations/${id}`
      );
    ...
    async deleteEdu(id) {
      ...
      this.eduDeleteData = {};
      this.isAddingEdu = false;
      this.eduDeleting = this.isDeletedModel = false;
    },
