<template>
  <div>
        ...
        <Modal v-model="isDeletedModel" width="360">
          ...
          <div slot="footer">
            <Button
              ...
              size="large"
              long
              @click="deleteSkill(skillDeleteData.id)"
              :loading="SkillDeleting"
    ...
    <div v-if="skill.skill" class="show-data">
      <p @click="addSkill = !addSkill" class="link-para">
    ...
    <div v-if="addSkill">
      ...
      <Form ref="skill" :model="skill" :rules="ruleValidate">
        ...
        <div v-if="skill.level">
          <p class="level">
            Level- <span :class="skill.level">{{ skill.level }}</span>
          </p>
          <br />
        </div>
    ...
    <div>
      <div class="open-form" @click="addSkills()">
  ...
  methods: {
          ...
          if (this.editId) {
            try {
              ...
              const res = await Axios.put(
                `/api/resumes/${this.resumeId}/skills/${this.editId}`,
                this.skill
              );
              await this.getResumeData();
            ...
            try {
              ...
              const res = await Axios.post(
                `/api/resumes/${this.resumeId}/skills`,
                this.skill
              );
              await this.getResumeData();
    ...
    async editSkill(id) {
      ...
      const { data } = await Axios.get(
        `/api/resumes/${this.resumeId}/skills/${id}`
      );
      this.addSkill = true;
