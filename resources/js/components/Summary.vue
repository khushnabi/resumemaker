  ...
  <div>
    <div v-if="summariesData.length > 0">
      <div v-for="(sum, i) in summariesData" :key="i">
        <div class="show-data">
          <p class="link-para">
            <span @click="editSkill(sum.id)">{{ sum.summary }}</span
            ><span>
              <Icon
                class="delete"
                size="20"
        ...
        <Modal v-model="isDeletedModel" width="360">
          ...
          <div slot="footer">
            <Button
              ...
              size="large"
              long
              @click="deleteSummary(sumData.id)"
              :loading="summaryDeleting"
        ...
        <FormItem>
          <div class="button-wrap">
            <Button
              v-if="summaryEdit"
              type="primary"
              :loading="sumarySending"
              @click="handleSubmit('summary')"
              >Update</Button
            >
    ...
    <div>
      <div class="open-form" @click="addSummries()">
  ...
  methods: {
          ...
          if (this.editId) {
            try {
              ...
              const res = await Axios.put(
                `/api/resumes/${this.resumeId}/summaries/${this.editId}`,
                this.summary
              );
              await this.getResumeData();
    ...
    async editSkill(id) {
      ...
      const { data } = await Axios.get(
        `/api/resumes/${this.resumeId}/summaries/${id}`
      );

      this.addSummary = true;
      this.summary.summary = data.summary;

      this.resume.summaries = this.resume.summaries.filter(
        (arr) => arr.id !== id
      );
      this.editId = id;
